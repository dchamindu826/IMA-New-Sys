...
import { ChevronLeft, ChevronRight, Calendar as CalendarIcon, Plus, Loader2, X, BookOpen, Trash2, Clock, CalendarDays, Search, ChevronDown, Building2, UserCircle } from 'lucide-react';
...
export default function ManagerTimetable() {
  ...
  const [scheduleForm, setScheduleForm] = useState({ subjectId: '', subjectName: '', content: '', startTime: '', endTime: '', lecturerId: '', extraTasks: [] });
      ...
      try {
        ...
        if (isAdmin) {
            ...
            if (uniqueBusinesses.length > 0) {
                const firstBusId = uniqueBusinesses[0].id.toString();
                setSelectedBusiness(firstBusId);
                const bList = fetchedBatches.filter(b => b.business_id.toString() === firstBusId);
                setFilteredBatches(bList);
                if(bList.length > 0) setSelectedBatch(bList[0].id.toString());
            }
        } else {
            setFilteredBatches(fetchedBatches);
            if(fetchedBatches.length > 0) setSelectedBatch(fetchedBatches[0].id.toString());
        }

        // 💡 URL එක හරියටම දුන්නා 💡
        const staffRes = await api.get('/tasks/manager/staff-list').catch(() => ({data: []}));
        setLecturers(staffRes.data.filter(u => u.role === 'Lecturer') || []);
      } catch (error) { toast.error("Failed to load initial data"); }
  ...
  useEffect(() => {
      ...
      if (isAdmin && selectedBusiness !== 'all') {
          ...
          if (allBatches.length > 0) setSelectedBatch(allBatches[0].id.toString());
          else setSelectedBatch('');
      }
  ...
  useEffect(() => {
    if(!selectedBatch) {
        setSubjects([]); setSchedules([]); return;
    }
    const fetchBatchDetails = async () => {
      const month = currentDate.getMonth() + 1;
      const year = currentDate.getFullYear();
      try {
        const batchDetails = allBatches.find(b => b.id.toString() === selectedBatch);
        let allSubjects = [];
  ...
  const openScheduleModal = () => {
    setScheduleForm({ subjectId: '', subjectName: '', content: '', startTime: '', endTime: '', lecturerId: '', extraTasks: [] });
  ...
  const handleSaveSchedule = async (e) => {
      ...
      if (!scheduleForm.lecturerId) return toast.error("Please select a lecturer!");

      try {
          const payload = { 
              batchId: selectedBatch, date: selectedDay, subject: scheduleForm.subjectName, 
              content: scheduleForm.content, startTime: scheduleForm.startTime, endTime: scheduleForm.endTime,
              lecturerId: scheduleForm.lecturerId 
          }; 
          const res = await api.post('/tasks/manager/schedule/add', payload);
          toast.success("Class Scheduled & Mandatory Tasks Auto-Assigned!");
          
          setSchedules([...schedules, { id: res.data?.id || Math.random(), date: selectedDay, title: scheduleForm.subjectName, start_time: scheduleForm.startTime, end_time: scheduleForm.endTime, content: scheduleForm.content }]);
      ...
      <div className="flex justify-between items-start mb-6">
        <div>
          <h2 className="text-3xl font-bold text-white drop-shadow-md flex items-center gap-3">
            <CalendarIcon className="text-blue-400" size={28}/> Master Timetable
          </h2>
        </div>
        ...
        <div className="flex gap-3">
            ...
            <div className="bg-slate-800/40 border border-white/10 p-2 rounded-2xl flex flex-col shadow-lg">
                ...
                <select className="bg-black/30 border border-white/10 text-white text-sm rounded-xl px-4 py-2 outline-none mt-1 cursor-pointer focus:border-blue-400" value={selectedBatch} onChange={(e) => setSelectedBatch(e.target.value)}>
                    {filteredBatches.length === 0 ? <option value="">No Batches</option> : filteredBatches.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
                </select>
            </div>
        </div>
      </div>
                            ...
                            <div key={day} onClick={() => { setSelectedDay(dateString); setShowDayViewModal(true); }} className={`bg-black/20 rounded-xl border p-2 cursor-pointer transition-all group relative min-h-[90px] flex flex-col ${isToday ? 'border-blue-500/50 shadow-[0_0_15px_rgba(59,130,246,0.2)]' : 'border-white/5 hover:border-blue-400/30'}`}>
                                ...
                                <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 space-y-1 mt-1 pointer-events-none">
                                    {daySchedules.slice(0, 3).map(sch => (
                                        <div key={sch.id} className="text-[9px] p-1.5 rounded border bg-blue-500/20 border-blue-500/30 text-blue-200 font-bold truncate">
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/70 backdrop-blur-md p-4">
          <div className="bg-slate-900 border border-white/20 rounded-3xl shadow-2xl w-full max-w-xl flex flex-col overflow-hidden">
            <div className="p-5 border-b border-white/10 flex justify-between items-center bg-black/40">
              <div>
                <h3 className="text-xl font-bold text-white mb-1">Schedule Class</h3>
                <p className="text-blue-400 text-xs font-bold uppercase tracking-widest">{selectedDay}</p>
              </div>
              ...
              <form id="schedule-form" onSubmit={handleSaveSchedule} className="space-y-5">
                  <div className="grid grid-cols-2 gap-5">
                        ...
                        {isDropdownOpen && (
                            <div className="absolute z-50 w-full mt-2 bg-slate-800 border border-white/10 rounded-xl shadow-2xl flex flex-col max-h-[200px]">
                                <div className="p-2 border-b border-white/10 bg-slate-900/50 rounded-t-xl sticky top-0">
                                    <input type="text" placeholder="Search..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onClick={e => e.stopPropagation()} className="w-full bg-transparent border-none p-1 text-sm text-white outline-none" />
                                </div>
                    ...
                    <div className="col-span-2">
                        <label className="text-xs font-bold text-slate-400 mb-1.5 block">Lecturer</label>
                        <select required value={scheduleForm.lecturerId} onChange={e => setScheduleForm({...scheduleForm, lecturerId: e.target.value})} className="w-full bg-black/40 border border-white/10 rounded-xl p-3 text-sm text-white outline-none focus:border-blue-400">
                           <option value="">-- Choose Lecturer --</option>
                           {lecturers.map(l => <option key={l.id} value={l.id}>{l.fName} {l.lName}</option>)}
                        </select>
                    </div>
                  </div>
                  <div>
                      <label className="text-xs font-bold text-slate-400 mb-1.5 block">Content to Cover</label>
                      <input required type="text" value={scheduleForm.content} onChange={e => setScheduleForm({...scheduleForm, content: e.target.value})} placeholder="E.g. Part 1 Discussion" className="w-full bg-black/40 border border-white/10 rounded-xl p-3 text-sm text-white outline-none focus:border-blue-400" />
                  </div>
                  <div className="grid grid-cols-2 gap-5">
            ...
            <div className="p-5 border-t border-white/10 bg-black/40">
               <p className="text-[10px] text-blue-400 mb-3 text-center font-bold uppercase">* 3 Mandatory Content Hub tasks will be automatically assigned to the Coordinator.</p>
               <button type="submit" form="schedule-form" className="w-full bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 text-white font-bold py-3.5 rounded-xl shadow-lg transition-all hover:scale-[1.01]">Confirm Schedule</button>
            </div>
          </div>
        </div>
      )}
                 ...
                 <div className="p-6 max-h-[60vh] overflow-y-auto custom-scrollbar space-y-4">
                     {schedules.filter(s => s.date === selectedDay).length === 0 ? (
                         <div className="text-center py-10">
                            ...
                            <button onClick={openScheduleModal} className="mt-4 bg-blue-600/20 text-blue-400 hover:bg-blue-600/40 px-4 py-2 rounded-lg text-sm font-bold transition-all">+ Schedule Class</button>
                         </div>
                     ) : (
                         schedules.filter(s => s.date === selectedDay).map(sch => (
                             <div key={sch.id} className="p-4 bg-slate-800/50 border border-white/10 rounded-2xl flex justify-between items-start">
                                 <div>
                                     <h4 className="text-lg font-bold text-white mb-1">{sch.subject || sch.title}</h4>
