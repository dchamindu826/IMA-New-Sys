...
const getCoordinatorOverview = async (req, res) => {
    try {
        ...
        const assignment = await prisma.batch_staff_assignments.findFirst({ where: { staff_id: parseInt(userId) }, orderBy: { created_at: 'desc' } });
        ...
        const todayClasses = await prisma.class_schedules.findMany({ where: { batch_id: batch.id, date: new Date(today) } });
        const myTasks = await prisma.daily_tasks.findMany({ where: { staff_id: parseInt(userId), is_completed: false } });

        return res.status(200).json(safeJson({ user, business, batch, groups, todayClasses, pendingTasksCount: myTasks.length }));
    } catch (error) { return res.status(500).json({ message: "Server Error" }); }
};
